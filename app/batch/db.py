...
def _fetch_existing(table: str, dates: List[str], select: str, date_col: str = "date") -> Dict[str, dict]:
    """既存データを日付キーで取得。ページネーション対応。"""
    if not dates:
        return {}
...
def _detect_revisions(
    ...
    existing = _fetch_existing(table, dates, select_cols, date_col=date_col)
